  ...
  return (
    <div style={{ padding: '20px 20px 0', background: `linear-gradient(180deg, ${mockBg} 0%, ${mockBg}88 100%)`, minHeight: 180, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ borderRadius: '8px 8px 0 0', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 12px 32px rgba(0,0,0,0.5)', position: 'relative', zIndex: 1 }}>
        <div style={{ height: 142, background: `linear-gradient(155deg, ${screen.colorB}66, ${screen.colorA}15)`, padding: cardImageUrl ? 0 : '12px 12px 0' }}>
          {cardImageUrl ? <img src={cardImageUrl} alt="" style={{ display: 'block', width: '100%', height: '100%', objectFit: 'cover' }} /> : <>
            <div style={{ height: 58, background: `${screen.colorB}88`, borderRadius: 3, marginBottom: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 4 }}><div style={{ height: 5, width: '55%', background: 'rgba(255,255,255,0.4)', borderRadius: 2 }} /><div style={{ height: 10, width: 42, background: screen.colorA, borderRadius: 2 }} /></div>
            <div style={{ display: 'flex', gap: 6 }}>{[screen.colorA, screen.colorB, screen.colorA].map((color, index) => <div key={index} style={{ flex: 1, height: 38, background: `${color}22`, borderRadius: 3, border: `1px solid ${color}22` }} />)}</div>
          </>}
        </div>
      </div>
    </div>
