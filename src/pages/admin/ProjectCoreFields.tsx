...
export default function ProjectCoreFields({ project, type, isWork, isNew, techInput, update, onTechChange }: CoreFieldsProps) {
  ...
  return (
    <div className="admin-project-form-grid">
      ...
      {type === 'website' && <ProjectField label="Website URL"><input type="url" inputMode="url" value={project.siteUrl === '#' ? '' : project.siteUrl ?? ''} onChange={event => update('siteUrl', event.target.value)} placeholder="https://yourwebsite.com" /><small>Paste the live website address. The marketplace “Visit website” link opens it in a new tab.</small></ProjectField>}
